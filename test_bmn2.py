...
def test_bmn(itr, dataset, args, model, logger, device):
    ...
    for counter, (features, labels, idx, frange) in tqdm(enumerate(dataset.load_test_fixed(shuffle=False))):
    ...
    for counter, (features, labels, idx, frange) in tqdm(enumerate(dataset.load_test_fixed(shuffle=False))):
        ...
        for c in range(args.num_class):
            ...
            for j in range(len(s)):
                if e[j] - s[j] >= 2:
                    segment_predict.append(
                        [idx, (s[j]+offset)/cur_len, (e[j]+offset)/cur_len, np.mean(elem[s[j] : e[j]]),
                        c, duration]
                    )
        instance_logits_stack.append(tmp)
        labels_stack.append(labels)
