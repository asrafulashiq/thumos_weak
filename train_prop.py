...
def MILL_atn(elements_cls, seq_len, labels, device):
    ...
    wb = 1.0 / 20  # B, 1
    milloss = torch.mean(
        -torch.sum(labels * torch.log(tap[..., 1:] + 1e-4) + wb * torch.log(tap[..., [0]] + 1e-4), dim=-1)
    )
    ...
    for i in range(labels.shape[0]):  # process each batch
        ...
        elem_max = torch.max(elem, -1, keepdim=True)[0]  # T, 1
    ...
    loss_cont = torch.sum(loss_cont) / labels.shape[0]
...
def train_lpat(itr, dataset, args, model, optimizer, logger, device):
    ...
    print(
        f"{itr: >10d}: {t_val(milloss):.4f} + {t_val(loss_cont): .4f} = {t_val(total_loss): .4f}"
    )
